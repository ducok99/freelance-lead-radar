...
v0.5 — 2026-07-20 — P4 có 50 test trong workers/api; toàn repo 364 test; Workers API line coverage 95,99%, adapter 97,29%, rules-engine 98,17%.
...
Thứ tự hiện tại: `install` → `check-secrets` → `check-api-safety` → `typecheck` → `lint` (kèm rule an toàn) → `unit + contract` → `test:coverage`. Từ phase có extension sẽ thêm `build extension` và e2e phù hợp. Bất kỳ bước nào đỏ → không merge. CI chặn rules-engine nếu line coverage < 90%, facebook-adapter và workers/api nếu < 80%.
