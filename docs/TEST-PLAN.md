...
v0.2 — 2026-07-18 — đã được DUC duyệt; P1 hiện có 97 unit/contract tests toàn repo sau deep QA.
...
Thứ tự: `typecheck` → `lint` (kèm rule an toàn) → `unit + contract` → `build extension` → grep secret/từ khóa cấm (SECURITY §3) → (nightly, tùy chọn) e2e Playwright. Bất kỳ bước nào đỏ → không merge. Coverage mục tiêu: rules-engine ≥ 90% line, adapter ≥ 80%; các package khác theo dõi, không chặn trong MVP.
