...
v0.2 — 2026-07-18 — P0 đã duyệt; P1 hoàn thành local, chờ DUC nghiệm thu.
...
- [ ] CI đã chạy xanh trên GitHub Actions — **PENDING**: chỉ được đánh dấu hoàn thành khi có bằng chứng run xanh sau khi repo được push lên GitHub.
...
- [ ] ≥ 60 unit test; mỗi FilterReason có ≥ 2 test dương + ≥ 2 test âm.
- [ ] Bộ 40 bài mẫu tiếng Việt gán nhãn tay (fixture JSON, viết mới ở phase này): gate + hard filter pre-AI cho kết quả đúng ≥ 90% (đo bằng test, con số ghi trong báo cáo).
- [ ] `aggregateScore` có test bảng: tổng thành phần đúng, cap 94 khi confidence 0.84, không cap khi 0.85, không âm, không vượt 100.
- [ ] Counters: test vượt 10 bình luận/ngày → `daily_limit_reached`; test khoảng cách < 5 phút → chặn; test reset sang ngày mới.
- [ ] `transition()` từ chối mọi transition ngoài máy trạng thái (test liệt kê đủ cạnh hợp lệ).
- [ ] Package không import DOM API, không fetch (kiểm bằng eslint restriction cho riêng package + vitest setup chặn network).
...
- [ ] 100% fixture extract đủ trường bắt buộc (postKey, text, permalink); trường thiếu → null có chủ đích, không throw.
- [ ] Fixture "layout lạ" (HTML bị cắt xén chủ đích) → trả `ExtractionFailure` có mã, không throw.
- [ ] `parsePostKey` đúng cả 6+ dạng URL, kèm test URL rác → null.
- [ ] Adapter không export hàm submit comment nào (test tĩnh trên export surface — bất biến #15).
- [ ] README trong fixtures/ ghi quy trình làm sạch PII khi thêm fixture mới (theo TEST-PLAN §3).
