...
v0.2 — 2026-07-18 — cập nhật theo Phase 1. Zod schema trong `packages/shared` là nguồn chân lý; tài liệu này mô tả để người đọc không cần mở code. Mọi bản ghi lưu độc lập có `schemaVersion` để migrate về sau.
...
| apiBaseUrl, teamToken | string         | Cấu hình gọi Workers API; token rỗng hoặc ≥32 ký tự, không phải credential FB                   |
...
### FilterReason — map 1-1 với 9 HARD FILTERS trong spec
...
- Message nội bộ là discriminated union: gate state, post seen, warning detected, Emergency Stop, circuit reset, lead update và comment assist. Đây chỉ là hợp đồng dữ liệu; Phase 1 chưa có code tương tác Facebook.
