...
v0.6 — 2026-07-20 — P5 candidate có 17 unit/snapshot test riêng; toàn repo 381 test; extension controller 86,41%, content gate 89,58%, pure libs 85,49% line coverage.
...
| E2E-02 | Feed 10 bài trộn 4 loại                | Đúng N lead hiện side panel; bài loại có lý do trong dashboard |
| E2E-03 | Chạy lại cùng feed                     | 0 call API mới (dedupe)                                        |
| E2E-04 | Sửa nháp → duyệt → chèn trên permalink | Ô bình luận chứa đúng editedText; không auto-submit            |
...
Thứ tự hiện tại: `install` → `check-secrets` → `check-api-safety` → `typecheck` → `lint` (kèm rule an toàn) → `unit + contract` → `test:coverage` → `build extension` → cài Chromium → E2E ngoài allowlist. Bất kỳ bước nào đỏ → không merge. CI chặn rules-engine nếu line coverage < 90%; facebook-adapter, workers/api và extension nếu < 80%.
