...
v0.4 — 2026-07-20 — P3 có 55 test trong facebook-adapter; toàn repo 315 test; adapter line coverage 97,29%, rules-engine 98,17%.
...
Thứ tự hiện tại: `install` → `check-secrets` → `typecheck` → `lint` (kèm rule an toàn) → `unit + contract` → `test:coverage`. Từ phase có extension sẽ thêm `build extension` và e2e phù hợp. Bất kỳ bước nào đỏ → không merge. CI chặn rules-engine nếu line coverage < 90% và facebook-adapter nếu < 80%.
