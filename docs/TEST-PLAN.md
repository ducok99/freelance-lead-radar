...
v0.3 — 2026-07-20 — P2 có 161 test trong rules-engine; toàn repo 261 test; rules-engine line coverage 98,17%.
...
Thứ tự hiện tại: `install` → `check-secrets` → `typecheck` → `lint` (kèm rule an toàn) → `unit + contract` → `test:coverage`. Từ phase có extension sẽ thêm `build extension` và e2e phù hợp. Bất kỳ bước nào đỏ → không merge. CI đang chặn rules-engine nếu line coverage < 90%; adapter sẽ chặn ở mức ≥ 80% khi được triển khai.
